...
from libs_my.django_cache import cache, pop, fn, get_cache_model # 这样可以替换上面两行
...
class Company(models.Model):
    ...
    access_dt = models.DateField('日期', db_column='c_access_dt')
    add_dt = models.DateTimeField(auto_now_add=True, db_column='c_add_dt')
    update_dt = models.DateTimeField(auto_now=True, db_column='c_update_dt')
...
class Person(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, db_column='c_id')
    company = models.ForeignKey(Company, verbose_name='公司', null=True, db_column='c_company_id', on_delete=models.DO_NOTHING)
    name = models.CharField("name", max_length = 20)
    ...
    def __eq__(self, obj):
        res = (self.id == obj.id and
            self.company_id == obj.company_id and
            self.name == obj.name and
            self.age == obj.age and
            self.access_dt == obj.access_dt and
            self.created_at == obj.created_at and
            self.updated_at == obj.updated_at
            )
        if not res:
            logging.info('****** model 对比不相等 *******')
            if self.id != obj.id:
                logging.info('--- id --- %s %s %s %s', type(self.id), type(obj.id), self.id, obj.id)
            if self.company_id != obj.company_id:
                logging.info('--- company_id --- %s %s %s %s', type(self.company_id), type(obj.company_id), self.company_id, obj.company_id)
            if self.name != obj.name:
                logging.info('--- name --- %s %s %s %s', type(self.name), type(obj.name), self.name, obj.name)
            if self.age != obj.age:
                logging.info('--- age --- %s %s %s %s', type(self.age), type(obj.age), self.age, obj.age)
            if self.access_dt != obj.access_dt:
                logging.info('--- access_dt --- %s %s %s %s', type(self.access_dt), type(obj.access_dt), self.access_dt, obj.access_dt)
            if self.created_at != obj.created_at:
                logging.info('--- created_at --- %s %s %s %s', type(self.created_at), type(obj.created_at), self.created_at, obj.created_at)
            if self.updated_at != obj.updated_at:
                logging.info('--- updated_at --- %s %s %s %s', type(self.updated_at), type(obj.updated_at), self.updated_at, obj.updated_at)
        return res
...
class CacheModelTest(ModelTest):

    def test_get_cache_model(self):
        """get_cache_model 函数测试"""
        logging.info(u'get_cache_model 函数测试')
        key = 'my_cache_model_key'
        now = datetime.datetime.now()
        p = Person(id='21ff2785-6c57-46f8-b8e2-2c2aa66157cd', name="aa", age=111, access_dt=now, created_at=now, updated_at=now)
        p2 = get_cache_model(Person, key, lambda :p, timeout=1)
        assert id(p) == id(p2) # 相同的内存地址，取返回的p
        assert p == p2 # 交给 model 里面的 __eq__ 函数比较
        assert isinstance(p2.id, uuid.UUID)

        p3 = get_cache_model(Person, key, lambda :p, timeout=1)
        assert id(p) != id(p3) # 不同的内存地址，经过redis缓存取
        assert p == p3 # 交给 model 里面的 __eq__ 函数比较
        assert isinstance(p3.id, uuid.UUID)

    def test_get_cache_model2(self):
        """get_cache_model 函数测试"""
        logging.info(u'get_cache_model 函数第二次测试')
        key = 'my_cache_model_key2'
        now = datetime.datetime.now()
        company = Company(id='43fa4af1954342a0931e9a0dd70f2fc8', name='cc', access_dt='2018-03-05 21:59:00', add_dt=now, update_dt=now)
        p = Person(id='21ff2785-6c57-46f8-b8e2-2c2aa66157cd', name="bb", age=35, access_dt=now, created_at=now, updated_at=now, company=company)
        p2 = get_cache_model(Person, key, lambda :p, timeout=1)
        assert id(p) == id(p2) # 相同的内存地址，取返回的p
        assert p == p2 # 交给 model 里面的 __eq__ 函数比较
        assert isinstance(p2.id, uuid.UUID)
