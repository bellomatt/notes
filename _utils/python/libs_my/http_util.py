    ...
    try:
        ...
        # post、put、delete、patch、options 方式的参数处理
            ...
            if files:
                ...
                param = multiple_url_encode(param, files=files, boundary=boundary)
            elif send_json:
                if not param:
                    param = ''
...
def multiple_url_encode(param=None, files=None, boundary=None, encode="utf-8"):
    u"""
    将字典转成 url 参数
    :param {dict|string} param: 要提交到网页的参数(get方式会拼接到 url 上)
    :param {list<dict> | dict} files: 要提交到网页的文件列表,只有一个文件时可以传dict类型,多个文件传list<dict>类型
    ...
    :return {string}: 返回可以提交的字符串参数
    @example
        s = multiple_url_encode({'name' : '测试用户', 'password' : 123456})  # 返回:password=123456&name=%E6%B5%8B%E8%AF%95%E7%94%A8%E6%88%B7
    """
    if not boundary:
        return ''
