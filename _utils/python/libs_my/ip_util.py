...
def get_tornado_ip(request):
    ...
    client_ip = _get_remote_ip(request.remote_ip)
    if not client_ip:
        client_ip = _get_remote_ip(headers.get("Remote-Addr", ""))
    if not client_ip:
        client_ip = _get_remote_ip(headers.get("X-Real-Ip", ""))
    if not client_ip:
...
def get_django_ip(request):
    """
    获取django接收的请求过来的ip地址
    :param {django.request} request 必须是 django 的请求
    :return {string}: 发请求过来的IP地址
    """
    client_ip = _get_remote_ip(request.META.get("REMOTE_ADDR", ""))
    if not client_ip:
        client_ip = _get_remote_ip(request.META.get("HTTP_X_REAL_IP", ""))
    if not client_ip:
        client_ip = _get_remote_ip(request.META.get('HTTP_X_FORWARDED_FOR', ''))
    return client_ip
