...
Mysql 连接数
    1.查看当前连接数
        方式一：命令行调用 mysqladmin 查询
        ./mysqladmin -u用户名 -p密码 status
        展示： Uptime: 1370150  Threads: 1 （当前连接数） Questions: 79  Slow queries: 0  Opens: 33  Flush tables: 1  Open tables: 26  Queries per second avg: 0.000

        方式二：命令行调用 mysql 查询
        ./mysql -u用户名 -p密码 -e 'show status' | grep -i  Threads_running
        展示： Threads_running    1 ###（当前连接数）
    ...
    2.查看最大连接数
        方式一：命令行查询
        ./mysql -u用户名 -p密码 -e 'show variables' | grep max_connections
        展示： max_connections    500
