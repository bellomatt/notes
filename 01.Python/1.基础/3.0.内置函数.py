...
locals()
    更新并返回一个代表当前 local 符号表的字典。警告：本词典的内容不应该被修改，更改可能不会影响由interpreter用作局部变量的值。
...
ord(c)
    ...
    例如，ord('a')返回整数97，ord(u'\u2020')返回8224。这是8位串 chr()和用于Unicode对象的 unichr()的逆函数。如果给出Unicode参数和Python是UCS2 Unicode的，字符的代码点必须在范围[0 .. 65535]内，否则字符串的长度是2，抛出一个 TypeErro 。
