...
参考： https://www.runoob.com/xpath/xpath-functions.html#numeric
...
| 名称	| 说明 | 例子 | 结果 |
|---|---|---|---|
| fn:number(arg)	| 返回参数的数值。参数可以是布尔值、字符串或节点集。 | number('100') | 100
| fn:abs(num)	| 返回参数的绝对值。 | abs(3.14) | 3.14 | abs(-3.14) | 3.14
| fn:ceiling(num)	| 返回大于 num 参数的最小整数。 | ceiling(3.14) | 4
| fn:floor(num)	| 返回不大于 num 参数的最大整数。 | floor(3.14) | 3
| fn:round(num)	| 把 num 参数舍入为最接近的整数。 | round(3.14) | 3
| fn:round-half-to-even()	|  | round-half-to-even(0.5) <br/> round-half-to-even(1.5) <br/> round-half-to-even(2.5) | 0 <br/> 2 <br/> 2
...
| fn:codepoint-equal(comp1,comp2)	| 根据 Unicode 代码点对照，如果 comp1 的值等于 comp2 的值，则返回 true。否则返回 false。
...
| fn:resolve-uri(relative,base)	| (估计跟上面的 escape-uri 相反)
